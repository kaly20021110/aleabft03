...
func (h *Hasher) Sum256(data []byte) Digest { //生成256-bit的报文摘要
	defer func() {
		h.data = nil
	}()
...
type PublickKey struct { //基于椭圆曲线的数字签名方案的公钥
	Pubkey ed25519.PublicKey
}

type PrivateKey struct { //基于椭圆曲线的数字签名方案的私钥
	Prikey ed25519.PrivateKey
}

type SecretShareKey struct { //部分签名的公钥私钥以及总结点数量和允许出错的节点数量
...
type Signature struct { //完整签名本质是byte数组
	Sig []byte
}

func (s *Signature) Verify(pub PublickKey, digest Digest) bool { //使用公钥验证摘要签名
	return ed25519.Verify(pub.Pubkey, digest[:], s.Sig)
}

type SignatureShare struct { //部分签名的本质也是byte数组
	PartialSig []byte
}

func (s *SignatureShare) Verify(Digest) bool { //部分签名的验证，直接返回正确值吗？
	return true
}

func VerifyTs(skey SecretShareKey, digest Digest, intactSig []byte) error { //验证部分签名
	suite := bn256.NewSuite()
	err := bls.Verify(suite, skey.PubPoly.Commit(), digest[:], intactSig)
	return err
}

// CombineIntactTSPartial assembles the intact threshold signature.部分签名聚合称为完整签名 partsignature to fullsignature
...
func (s *SigService) RequestSignature(digest Digest) (Signature, error) { //生成一个完整签名
	req := &sigReq{
		typ:    SIG,
		digest: digest,
...
func (s *SigService) RequestTsSugnature(digest Digest) (SignatureShare, error) { //生成一个部分签名
	req := &sigReq{
		typ:    SHARE,
		digest: digest,
		Done:   make(chan *sigReq, 1),
